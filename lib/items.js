...
module.exports = {
    ...
    detail : async (req, res)=>{
        ...
        try{
            ...
            const colorList =  await query('SELECT value FROM colors WHERE item_id =?', [itemId]);
            const optionList =  await query('SELECT value FROM options WHERE item_id =?', [itemId]);
    ...
    update_item : async (req,res)=>{
        ...
        try{
            ...
            if(isDataInserted && isImageInserted){
                // 이미지 삭제 함수
                const thumbDeleteResult = await query('DELETE FROM item_image WHERE item_id = ? AND type = "thumbnail" AND created_at < NOW() - INTERVAL 15 SECOND', [itemId]);
                const detailDeleteResult = await query('DELETE FROM item_image WHERE item_id = ? AND type = "detail" AND created_at < NOW() - INTERVAL 15 SECOND', [itemId]);
                // 옵션 삭제 함수
                const colorsDeleteResult = await query('DELETE FROM colors WHERE item_id = ? AND created_at < NOW() - INTERVAL 15 SECOND', [itemId])
                const optionsDeleteResult = await query('DELETE FROM options WHERE item_id = ? AND created_at < NOW() - INTERVAL 15 SECOND', [itemId])
            }
